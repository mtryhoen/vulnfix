...
- **Handle multi-version Trivy fixes**: when Trivy reports `FixedVersion: "1.3.3, 0.3.85"` (a CVE patched on multiple release lines), the adapter now picks the candidate closest to the installed major/minor line and never downgrades. Previously the raw comma-joined string was passed to `uv lock`, which failed.
